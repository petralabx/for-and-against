...
A **separate** Attribution campaign, `For & Against — Google Ads`, keeps Google-driven performance isolated from the existing `For & Against Website` campaign (see the site's own Attribution structure, TODO: link once `docs/channels/amazon-attribution.md` exists).
